...
raw_dataset_dir = '/media/sdb1/carla_dataset'
...
for i in range(len(sequences)):
    ...
    os.system(f'ln -s {raw_dataset_dir}/Val/{sequences[i]}/{coordinate} {os.path.join(sequence_dir, str(i+len_train).zfill(2))}')
    # generate soft links
...
for i in range(len(sequences)):
    ...
    os.system(f'ln -s {raw_dataset_dir}/Test/{sequences[i]}/{coordinate} {os.path.join(sequence_dir, str(i+len_train_val).zfill(2))}')
    # generate soft links
train_val_test_split.append(len(sequences))
